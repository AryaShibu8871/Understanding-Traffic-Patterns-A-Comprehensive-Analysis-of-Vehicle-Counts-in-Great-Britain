...
**LSTM (Long Short‑Term Memory)** LSTM networks were used because traffic data is highly temporal and contains long‑term patterns such as seasonal trends, rush‑hour cycles, and yearly variations. LSTMs are excellent at remembering information over long sequences, making them ideal for forecasting vehicle counts based on historical data. **GRU (Gated Recurrent Unit)** GRUs were included as a lighter, faster alternative to LSTMs. They capture the same temporal dependencies but with fewer parameters, which reduces training time while maintaining strong predictive performance. This makes GRUs well‑suited for large datasets and real‑time forecasting scenarios. **SVM (Support Vector Machine)** SVM regression was used to model non‑linear relationships between features such as location, road type, and time. SVMs are robust, handle high‑dimensional data well, and perform effectively when the relationship between inputs and outputs is complex. They provide a strong baseline to compare against deep learning models.
